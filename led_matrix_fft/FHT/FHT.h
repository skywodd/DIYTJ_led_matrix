...
const PROGMEM  int16_t _cas_constants[]  = {
#if (FHT_N ==  256)
...
#if (REORDER == 1)
  const PROGMEM  uint8_t _reorder_table[]  = {
  #if (FHT_N == 256)
  ...
  const PROGMEM  uint8_t _log_table[]  = {
    #include <decibel.inc>
  };
#endif
...
#if (LIN_OUT == 1)
  const PROGMEM  uint8_t _lin_table[]  = {
...
#if (LIN_OUT8 == 1)
  const PROGMEM  uint8_t _lin_table8[]  = {
  ...
  const PROGMEM  int16_t _window_func[]  = {
  #if (FHT_N ==  256)
